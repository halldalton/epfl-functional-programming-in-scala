...
trait Solver extends GameDef {
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = {
    def blockNeighbors(neighbors: List[(Block, Move)]): LazyList[(Block, List[Move])] = {
      if (neighbors.isEmpty) LazyList.empty
      else (neighbors.head._1, neighbors.head._2 :: history) #:: blockNeighbors(neighbors.tail)
    }
    blockNeighbors(b.legalNeighbors)
  }
  ...
  def from(initial: LazyList[(Block, List[Move])],
    ...
    if (initial.isEmpty) LazyList.empty
      ...
      val more = for {
        init <- initial
        neighbors <- neighborsWithHistory(init._1, init._2)
      } yield neighbors
      val newNeighbors = newNeighborsOnly(more, explored)
