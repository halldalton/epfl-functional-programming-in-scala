import funsets.{FunSets, FunSetsInterface}
...
trait FunSets extends FunSetsInterface {
   ...
   */
  def filter(s: FunSet, p: Int => Boolean): FunSet = (x: Int) => if (p(x)) s(x) else false
...
def set1: Int => Boolean = (x: Int) => x < -995
FunSets.toString(FunSets.map(set1, x => x + 1))
